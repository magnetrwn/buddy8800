...
void pty::open() {
    if (master_fd >= 0)
        throw std::logic_error("PTY already open");
    try {
        ...
        if (error != 0)
            throw std::system_error(error, std::generic_category(), "ptsname_r");
...
const char* pty::name() const {
    return master_fd < 0 ? "" : slave_device_name;
}

bool pty::try_getch(u8& byte) const {
    ssize_t result;
    do {
        result = ::read(master_fd, &byte, 1);
    } while (result < 0 && errno == EINTR);
...
char pty::getch() const {
    ...
    if (echo_received_back)
        putch(static_cast<char>(byte));
    return static_cast<char>(byte);
}

void pty::putch(char byte) const {
    while (!try_putch(static_cast<u8>(byte)))
        wait_for(master_fd, POLLOUT);
}
...
bool pty::poll() const {
    if (master_fd < 0)
        throw std::logic_error("PTY is closed");
...
void pty::recv(char* data, usize max, char terminator) const {
    if (max == 0)
        throw std::invalid_argument("recv buffer must not be empty");
    if (max == 1) {
        data[0] = getch();
        return;
    }
...
void pty::setup(u32 data_bits, pty_parity parity, u32 stop_bits) {
    ...
    tty.c_cc[VMIN] = 1;
    tty.c_cc[VTIME] = 0;
    if (tcsetattr(slave_fd, TCSANOW, &tty) < 0)
        fail("tcsetattr");
}
...
void pty::set_baud_rate(u32 baud_rate) {
    ...
    switch (baud_rate) {
        ...
        case 115200:
            ...
            break;
        default:
            throw std::invalid_argument("Unsupported PTY baud rate");
    }
    termios tty{};
    if (tcgetattr(slave_fd, &tty) < 0)
        fail("tcgetattr");
